      ...
      <div className="noise mx-auto grid max-w-6xl grid-cols-1 items-center gap-10 px-4 py-16 sm:px-6 sm:py-20 lg:grid-cols-12 lg:py-24">
        ...
        >
          ...
          <h1 className="mt-5 text-4xl font-semibold tracking-tight sm:text-5xl lg:text-6xl">
            {t.hero.title1}
            <br />
            <span className="text-[hsl(var(--brand-orange-2))]">{t.hero.title2}</span>
          </h1>
          ...
          <div className="mt-7 flex flex-col gap-3 sm:flex-row">
            <Link to="/contact" data-testid="hero-contact-button">
              <Button className="h-11 w-full gap-2 bg-[hsl(var(--brand-orange))] px-7 text-white transition-[background-color,box-shadow] duration-200 hover:bg-[hsl(var(--brand-orange-2))] hover:shadow-md sm:w-auto">
                {t.common.contactUs}
        ...
        >
          <div className="relative overflow-hidden rounded-2xl border shadow-lg">
            <img
              ...
              loading="eager"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-[hsl(var(--brand-navy))]/70 via-transparent to-transparent" />
            <svg
...
export const StatsStrip = () => {
  ...
  return (
    <section className="mx-auto max-w-6xl px-4 sm:px-6">
      <motion.div {...fadeUp} className="grid grid-cols-2 gap-3 lg:grid-cols-4" data-testid="stats-strip">
        {t.stats.map((s, i) => {
          const Icon = icons[i];
          return (
            <Card key={i} className="border bg-card transition-[box-shadow,border-color] duration-200 hover:border-foreground/15 hover:shadow-md">
        ...
        <motion.div {...fadeUp}>
          <div className="relative overflow-hidden rounded-2xl border shadow-md">
            <img
              ...
              loading="lazy"
            />
            <div className="absolute inset-0 bg-[hsl(var(--brand-navy))]/10" />
          </div>
        </motion.div>
      </div>
    </section>
          ...
          {t.shippers.items.map((item, i) => {
            ...
            return (
              <motion.div key={i} {...fadeUp} transition={{ ...fadeUp.transition, delay: i * 0.06 }}>
                <Card className="h-full border bg-card transition-[box-shadow,border-color] duration-200 hover:border-foreground/15 hover:shadow-md">
                  <CardContent className="p-6">
                    <span className="flex h-11 w-11 items-center justify-center rounded-lg bg-[hsl(var(--brand-navy))] text-white">
...
export const CommitmentSection = () => {
  ...
  return (
    <section
      className="bg-[hsl(var(--brand-navy))] py-16 text-white sm:py-20"
          ...
          <Card className="h-full border bg-[hsl(var(--brand-sand))]/60 transition-[box-shadow] duration-200 hover:shadow-md">
            <CardContent className="flex h-full flex-col justify-between p-7">
              ...
              <Link to="/contact" data-testid="partners-cta-button" className="mt-6 inline-block">
                <Button className="gap-2 bg-[hsl(var(--brand-navy))] text-white hover:bg-[hsl(var(--brand-navy-2))]">
                  {t.partners.cta}
                  <ArrowRight className="h-4 w-4" />
                </Button>
              </Link>
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
          {t.fleet.items.map((item, i) => {
            ...
            return (
              <motion.div key={i} {...fadeUp} transition={{ ...fadeUp.transition, delay: i * 0.06 }}>
                <Card
                  className="group h-full overflow-hidden border bg-card transition-[box-shadow,border-color] duration-200 hover:border-[hsl(var(--brand-orange))]/50 hover:shadow-md"
                  data-testid={`fleet-card-${item.code.toLowerCase()}`}
                >
                  <div className="h-1.5 bg-[hsl(var(--brand-orange))]" />
                  <CardContent className="p-5">
                    <span className="flex h-12 w-12 items-center justify-center rounded-lg bg-[hsl(var(--brand-navy))] text-white">
...
export const ValuePropsStrip = () => {
  const { t } = useLang();
  return (
    <section className="border-y bg-card" data-testid="value-props-strip">
      <div className="mx-auto grid max-w-6xl grid-cols-1 gap-x-6 gap-y-4 px-4 py-8 sm:grid-cols-2 sm:px-6 lg:grid-cols-5">
          ...
          return (
            <div key={i} className="flex items-center gap-3">
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[hsl(var(--brand-sand))]">
                <Icon className="h-4.5 w-4.5 text-[hsl(var(--brand-orange-2))]" size={18} strokeWidth={2} />
              </span>
              <p className="text-sm font-semibold leading-snug">{v}</p>
            </div>
          );
        })}
      </div>
        ...
        <Card className="overflow-hidden border">
          <CardContent className="p-0">
            <div className="grid grid-cols-1 lg:grid-cols-2">
              {/* Main branches */}
              <div className="border-b p-7 lg:border-b-0 lg:border-r sm:p-9">
                ...
                <div className="mt-5 flex flex-wrap gap-2.5">
                  {t.network.branches.map((b, i) => (
                    <Badge
                      ...
                      className="rounded-full bg-[hsl(var(--brand-navy))] px-4 py-1.5 text-sm font-medium text-white hover:bg-[hsl(var(--brand-navy-2))]"
                    >
                      {b}
                    </Badge>
                  ))}
                ...
                <div className="mt-5 flex items-center gap-2 text-xs text-muted-foreground">
                  <span className="h-2.5 w-2.5 rounded-full bg-[hsl(var(--brand-navy))]" />
              ...
              <div className="p-7 sm:p-9">
                <div className="flex items-center gap-2.5">
                  <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-[hsl(var(--brand-orange))] text-white">
                    <Network className="h-4.5 w-4.5" size={18} />
                  </span>
